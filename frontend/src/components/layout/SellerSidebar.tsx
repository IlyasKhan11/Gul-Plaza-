import { NavLink, useNavigate } from 'react-router-dom'
import { FiGrid, FiPackage, FiShoppingBag, FiDollarSign, FiBriefcase, FiSettings, FiGlobe, FiLogOut } from 'react-icons/fi'
import { cn } from '@/lib/utils'
import { useAuth } from '@/context/AuthContext'

const navItems = [
  { to: '/seller/dashboard', icon: FiGrid, label: 'Dashboard' },
  { to: '/seller/products', icon: FiPackage, label: 'Products' },
  { to: '/seller/orders', icon: FiShoppingBag, label: 'Orders' },
  { to: '/seller/earnings', icon: FiDollarSign, label: 'Earnings' },
  { to: '/seller/withdrawals', icon: FiBriefcase, label: 'Withdrawals' },
  { to: '/seller/store', icon: FiSettings, label: 'Store Settings' },
]

export function SellerSidebar({ onLinkClick }: { onLinkClick?: () => void }) {
  const { user, logout } = useAuth()
  const navigate = useNavigate()

  if (!user || user.role !== 'seller') return null

  async function handleLogout() {
    onLinkClick?.()
    await logout()
    navigate('/')
  }

  return (
    <aside className="w-60 shrink-0 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 flex flex-col h-full overflow-y-auto">
      <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center">
            <FiGlobe className="h-4 w-4 text-white" />
          </div>
          <div>
            <p className="text-sm font-bold text-slate-900 dark:text-slate-100">Seller Panel</p>
            <p className="text-xs text-slate-400 dark:text-slate-500">Manage your store</p>
          </div>
        </div>
      </div>

      <nav className="flex-1 p-3 space-y-0.5">
        <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider px-3 pt-2 pb-1.5">Navigation</p>
        {navItems.map(({ to, icon: Icon, label }) => (
          <NavLink
            key={to}
            to={to}
            onClick={onLinkClick}
            className={({ isActive }) =>
              cn(
                'flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all',
                isActive
                  ? 'bg-blue-600 text-white shadow-sm'
                  : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-100'
              )
            }
          >
            {({ isActive }) => (
              <>
                <Icon className={cn('h-4 w-4 shrink-0', isActive ? 'text-blue-100' : 'text-slate-400 dark:text-slate-500')} />
                {label}
              </>
            )}
          </NavLink>
        ))}
      </nav>

      <div className="p-3 border-t border-slate-100 dark:border-slate-700">
        <button
          onClick={handleLogout}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
        >
          <FiLogOut className="h-4 w-4 shrink-0" />
          Logout
        </button>
      </div>
    </aside>
  )
}
